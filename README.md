...
- The macOS binary was compiled on High Sierra (v10.13.6). Known Issue: Columns are also sorted when clicking on the first row, and not only by clicking on the title rows (normal way). Seems to be an issue with the TListView component on macOS.
